...
import QuizInfo from './../Coach/QuizInfo';
import './createMentorQuiz.css';
function CreateMentorQuiz(props) {
    const [teamMentorID, setTeamMentorID] = useState('');
    ...
    const createMentorQuiz = async () => {
        try {
            ...
            var tmpData = {questions: questions, author_id: teamMentorID, name: quizName, cat: category, lvl: level}
    ...
    useEffect(() => {
        setTeamMentorID(localStorage.getItem("_id"));
    }, []) 
...
export default CreateMentorQuiz;
